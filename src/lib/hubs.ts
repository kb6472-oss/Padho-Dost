...
export const getSubjectHub = cache(async (examSlug: string, subjectSlug: string) => {
  ...
  const [testByChapter, sectionalTest] = await Promise.all([
    chapterTestsForExam(subject.examId),
    // The whole-subject "section" test (generated by scripts/generate-sectional-tests.ts).
    prisma.mockTest.findFirst({
      where: { examId: subject.examId, slug: `sectional-${subject.slug}`, type: "SECTIONAL" },
  ...
  return {
    ...
    chapters,
    sectional: sectionalTest
      ? { id: sectionalTest.id, questions: sectionalTest._count.questions, durationMinutes: sectionalTest.durationMinutes }
      : null,
