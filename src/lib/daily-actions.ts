...
import { getDailyQuestion, getDailyGk } from "@/lib/daily";

// Grade the Daily Challenge / Daily GK SERVER-SIDE (the correct answer is never sent to the
// client before the student picks) and keep the streak alive for logged-in students.
export async function submitDaily(
  date: string,
  selectedOptionId: string,
  track: "daily" | "gk" = "daily",
): Promise<{ correctOptionId: string | null; isCorrect: boolean; explanation: string | null }> {
  const q = track === "gk" ? await getDailyGk(date) : await getDailyQuestion(date);
